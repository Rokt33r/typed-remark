...
describe('remarkGithub', () => {
  it('should wrap mentions in `strong` by default', () => {
...
/* Shortcut to process. */
function github (value: string, repo: Repository) {
  var options;
  ...
  return unified().use(remarkParse).use(remarkGitHub, options).processSync(value).toString();
}
